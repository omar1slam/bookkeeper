...
function buildSystemPrompt(vacationAlias) {
  const vacationAliasLines = vacationAlias
    ? [
        `The current vacation/trip is named "${vacationAlias}". Any message mentioning "${vacationAlias}" is a VACATION expense: category_key null, section "VACATION", with a short label (do not include "${vacationAlias}" in the label).`,
      ]
    : [];
  return [
 ...
 */
export async function parseMessage(text, { todayISO, tz, defaultCurrency = "EGP", vacationAlias = null } = {}) {
  const provider = (process.env.LLM_PROVIDER || "anthropic").toLowerCase();
  const system = buildSystemPrompt(vacationAlias);
