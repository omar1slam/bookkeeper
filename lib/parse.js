...
function buildSystemPrompt(vacationContext) {
  const { alias, labels = [] } = vacationContext || {};
  const vacationAliasLines = alias
    ? [
        `The current vacation/trip is named "${alias}". Any message mentioning "${alias}" is a VACATION expense: category_key null, section "VACATION", with a short label (do not include "${alias}" in the label).`,
        ...(labels.length
          ? [
              `Existing VACATION rows: ${labels.join(", ")}. When a vacation expense fits one of these, reuse that EXACT text as the label and put the specific item in note. e.g. with a "Utilities" row, "200 sahel utilities sunscreen" -> label "Utilities", note "sunscreen".`,
            ]
          : []),
      ]
    : [];
  return [
    ...
    'Vacation shorthand: "f&b" / "fnb" / "food and bev" -> label "Food & Beverage". Reuse conventional labels rather than inventing variants.',
    'ANY food or drink expense on vacation (meals, restaurants, snacks, bread, groceries, coffee, water, delivery, etc.) -> label "Food & Beverage", with the specific item in note.',
    'If the user says "put it in X" / "under X" / "in X" / "to X", X is the target label: use it verbatim as label and keep the rest of the description as note.',
    ...vacationAliasLines,
    "",
    ...
    'Input: "254 sahel f&b" (when the current trip is "sahel") -> {"category_key":null,"section":"VACATION","label":"Food & Beverage","amount":254}.',
    'Input: "140 sahel bread" (when the current trip is "sahel") -> {"category_key":null,"section":"VACATION","label":"Food & Beverage","amount":140,"note":"bread"}.',
    'Input: "200 sahel utilities sunscreen" (trip "sahel", existing "Utilities" row) -> {"category_key":null,"section":"VACATION","label":"Utilities","amount":200,"note":"sunscreen"}.',
    'Input: "200 sahel sunscreen put it in utilities" -> {"category_key":null,"section":"VACATION","label":"Utilities","amount":200,"note":"sunscreen"}.',
...
async function callAnthropic(system, user) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      model,
      max_tokens: 1024,
      // cache_control: the static system prompt is reused across messages within the
      // cache TTL. Note: below the model's minimum cacheable prefix (4096 tokens on
      // Haiku) the marker is silently ignored — free either way, verify via the usage log.
  ...
  if (data.usage) console.log("anthropic usage:", JSON.stringify(data.usage));
  return (data.content || [])
 ...
 */
export async function parseMessage(text, { todayISO, tz, defaultCurrency = "EGP", vacationContext = null } = {}) {
  const provider = (process.env.LLM_PROVIDER || "anthropic").toLowerCase();
  const system = buildSystemPrompt(vacationContext);
