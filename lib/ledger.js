...
/** ISO string for a ledger date cell; converts Sheets serial numbers defensively. */
function ledgerDateISO(v) {
  if (v == null) return "";
  if (typeof v === "number" && v > 25569) {
    // Sheets serial days since 1899-12-30; 25569 = 1970-01-01.
...
export async function sumForDate(dateISO) {
  ...
  let count = 0;
  for (const r of rows) {
    if (ledgerDateISO(r[COL.date]) === dateISO) {
...
/** Sum entries with startISO <= date <= endISO, plus a per-section breakdown. */
export async function sumBetween(startISO, endISO) {
  const rows = await readAllRows();
  let total = 0;
  let count = 0;
  const bySection = {};
  for (const r of rows) {
    const d = ledgerDateISO(r[COL.date]);
    if (d >= startISO && d <= endISO) {
      const amt = Number(r[COL.amount]) || 0;
      total += amt;
      count += 1;
      const sec = String(r[COL.section] || "other");
      bySection[sec] = (bySection[sec] || 0) + amt;
    }
  }
